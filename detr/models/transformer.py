...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        intermediate = []
        attn_weights = [] ## changes here

        for layer in self.layers: 
            output, attn = layer(output, memory, tgt_mask=tgt_mask, ## changes here
                           memory_mask=memory_mask,
                           tgt_key_padding_mask=tgt_key_padding_mask,
                           memory_key_padding_mask=memory_key_padding_mask,
                           pos=pos, query_pos=query_pos)
            attn_weights.append(attn) ## changes here
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(attn_weights) ## changes here

        return output.unsqueeze(0), torch.stack(attn_weights) ## changes here
